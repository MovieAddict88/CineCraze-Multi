...
public class DataRepository {
        ...
        apiService.getPlaylistIndex().enqueue(new Callback<PlaylistIndex>() {
            ...
            public void onResponse(Call<PlaylistIndex> call, Response<PlaylistIndex> response) {
                if (response.isSuccessful() && response.body() != null && response.body().getPlaylists() != null) {
                    ...
                    if (remoteVersion > localVersion) {
                        Log.d(TAG, "New version found. Remote: " + remoteVersion + ", Local: " + localVersion);
                        List<String> newPlaylistUrls = new ArrayList<>(remotePlaylistUrls);
                        if (localPlaylistUrls != null) {
                            newPlaylistUrls.removeAll(localPlaylistUrls);
                        }

                        if (!newPlaylistUrls.isEmpty()) {
                             Log.d(TAG, "Found " + newPlaylistUrls.size() + " new playlists to download.");
                            fetchAllPlaylists(newPlaylistUrls, remoteVersion, remotePlaylistUrls, callback);
                        } else {
                             Log.d(TAG, "No new playlists to download, but version is higher. Re-caching all.");
                             fetchAllPlaylists(remotePlaylistUrls, remoteVersion, remotePlaylistUrls, callback);
                        }
                    } else {
                    ...
                    callback.onError("Failed to load playlist index.");
                }
            }
    ...
    private void fetchAllPlaylists(List<String> playlistUrls, int version, List<String> allRemoteUrls, DataCallback finalCallback) {
        ...
        if (playlistUrls.isEmpty()) {
            cacheData(new ArrayList<>(), version, allRemoteUrls);
    ...
    private void cacheData(List<EntryWithCategory> entriesWithCategories, int version, List<String> allPlaylistUrls) {
        try {
