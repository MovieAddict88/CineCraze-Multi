...
public class VideoServerUtils {
    ...
    public static final String VIDEASY_BASE = "https://player.videasy.net";
    public static final String VIDSRC_WIN_BASE = "https://vidsrc.win";
    public static final String VIDFAST_PRO_BASE = "https://vidfast.pro";
    public static final String TWEMBED_STREAM_BASE = "https://www.2embed.stream";
    public static final String VIDPLUS_TO_BASE = "https://player.vidplus.to";
    ...
    public static final String[] EMBED_SERVERS = {
        ...
        "player.videasy.net", "vidsrc.win", "vidfast.pro", "2embed.stream", "player.vidplus.to"
    };
    ...
    public static String enhanceVideoUrl(String originalUrl) {
        ...
        } else if (originalUrl.contains("player.videasy.net")) {
            return addVideasyParameters(originalUrl);
        } else if (originalUrl.contains("vidsrc.win")) {
            return addVidSrcWinParameters(originalUrl);
        } else if (originalUrl.contains("vidfast.pro")) {
            return addVidFastProParameters(originalUrl);
        } else if (originalUrl.contains("2embed.stream")) {
            return add2embedStreamParameters(originalUrl);
        } else if (originalUrl.contains("player.vidplus.to")) {
            return addVidplusToParameters(originalUrl);
        }
    ...
    private static String addVidlinkProParameters(String url) {
        ...
        enhancedUrl.append("autoplay=true");
    ...
    private static String addVideasyParameters(String url) {
        StringBuilder enhancedUrl = new StringBuilder(url);
        if (!url.contains("?")) {
            enhancedUrl.append("?");
        } else {
        ...
        enhancedUrl.append("autoplay=1");
        Log.d(TAG, "Enhanced Videasy URL: " + enhancedUrl.toString());
        return enhancedUrl.toString();
    }
    
    private static String addVidSrcWinParameters(String url) {
        StringBuilder enhancedUrl = new StringBuilder(url);
        if (!url.contains("?")) {
            enhancedUrl.append("?");
        } else {
        ...
        enhancedUrl.append("autoplay=1");
        Log.d(TAG, "Enhanced VidSrc.win URL: " + enhancedUrl.toString());
        return enhancedUrl.toString();
    }
...
private static String addVidFastProParameters(String url) {
    ...
    private static String add2embedStreamParameters(String url) {
        ...
        enhancedUrl.append("autoplay=1");
        Log.d(TAG, "Enhanced 2embed.stream URL: " + enhancedUrl.toString());
        return enhancedUrl.toString();
    }

    private static String addVidplusToParameters(String url) {
        StringBuilder enhancedUrl = new StringBuilder(url);
        if (!url.contains("?")) {
            enhancedUrl.append("?");
        } else {
        ...
        enhancedUrl.append("autoplay=true");
        Log.d(TAG, "Enhanced player.vidplus.to URL: " + enhancedUrl.toString());
        return enhancedUrl.toString();
    }
...
private static String addGoogleDriveParameters(String url) {
    ...
    public static String buildEmbedUrl(String server, String tmdbId, String imdbId, String type, int season, int episode) {
        switch (server.toLowerCase()) {
            ...
            case "videasy":
                if (type.equals("movie")) {
                    return VIDEASY_BASE + "/movie/" + tmdbId;
                } else {
                    return VIDEASY_BASE + "/tv/" + tmdbId + "/" + season + "/" + episode;
                }
            case "vidsrcwin":
                if (type.equals("movie")) {
                    return VIDSRC_WIN_BASE + "/movie/" + tmdbId;
                } else {
                    return VIDSRC_WIN_BASE + "/tv/" + tmdbId + "/" + season + "/" + episode;
                }
            case "vidfastpro":
                if (type.equals("movie")) {
                    return VIDFAST_PRO_BASE + "/movie/" + tmdbId;
                } else {
                    return VIDFAST_PRO_BASE + "/tv/" + tmdbId + "/" + season + "/" + episode;
                }
            default:
                return null;
        }
    ...
    public static String getServerType(String url) {
        ...
        if (url.contains("player.videasy.net")) return "Videasy";
        if (url.contains("vidsrc.win")) return "VidSrcWin";
        if (url.contains("vidfast.pro")) return "VidFastPro";
        if (url.contains("2embed.stream")) return "2embed";
        if (url.contains("player.vidplus.to")) return "Vidplus";
