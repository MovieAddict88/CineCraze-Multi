...
public abstract class BaseFragment extends Fragment {
    ...
    private void triggerBackgroundRefreshIfNeeded() {
        ...
        final int beforeVersion = dataRepository.getLocalDataVersion();
        dataRepository.forceRefreshData(new DataRepository.DataCallback() {
            @Override
            public void onSuccess(List<Entry> entries) {
                int afterCount = dataRepository.getTotalEntriesCount();
                int afterVersion = dataRepository.getLocalDataVersion();
                boolean dataChanged = (afterCount != beforeCount) || (afterVersion != beforeVersion);
                if (dataChanged && getActivity() != null) {
                    getActivity().runOnUiThread(() -> {
                        currentPage = 0;
                        loadPageData();
                    });
