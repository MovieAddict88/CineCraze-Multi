...
/**
 * Optimized adapter for segmented entries
 * Handles large datasets efficiently with minimal memory usage
 */
public class SegmentedMovieAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final int VIEW_TYPE_ITEM = 0;
    private static final int VIEW_TYPE_PAGINATION = 1;
    
    private Context context;
    private List<SegmentedEntry> entryList;
    ...
    public SegmentedMovieAdapter(Context context, List<SegmentedEntry> entryList, boolean isGridView) {
    ...
    public void setEntryList(List<SegmentedEntry> entryList) {
        this.entryList = entryList;
        notifyDataSetChanged();
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_PAGINATION) {
            ...
            return new SegmentedMovieViewHolder(view);
        }
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof SegmentedMovieViewHolder) {
            SegmentedMovieViewHolder movieHolder = (SegmentedMovieViewHolder) holder;
            SegmentedEntry entry = entryList.get(position);

            movieHolder.title.setText(entry.getTitle());
            
            // Use Glide with optimized settings for better memory management
            Glide.with(context)
                .load(entry.getPoster())
                .placeholder(R.drawable.placeholder_poster) // Add placeholder
                .error(R.drawable.error_poster) // Add error image
                .centerCrop()
                .into(movieHolder.poster);
                
            movieHolder.rating.setRating(entry.getRatingFloat());

            if (movieHolder.description != null) {
                movieHolder.description.setText(entry.getDescription());
            }
            if (movieHolder.year != null) {
                movieHolder.year.setText(String.valueOf(entry.getYearInt()));
            }
            ...
            // Set episode count badge for series
            if (movieHolder.episodeBadge != null && entry.isHasSeasons()) {
                movieHolder.episodeBadge.setVisibility(View.VISIBLE);
                movieHolder.episodeBadge.setText(entry.getSeasonCount() + "S/" + entry.getEpisodeCount() + "E");
            } else if (movieHolder.episodeBadge != null) {
                movieHolder.episodeBadge.setVisibility(View.GONE);
            }

            movieHolder.itemView.setOnClickListener(v -> {
                Intent intent = new Intent(context, DetailsActivity.class);
                
                // For segmented entries, we pass the ID and load full data in DetailsActivity
                intent.putExtra("entryId", entry.getId());
                intent.putExtra("isSegmented", true);
    ...
    public class SegmentedMovieViewHolder extends RecyclerView.ViewHolder {
        ImageView poster;
        TextView title;
        RatingBar rating;
        ...
        TextView country;
        TextView duration;
        TextView categoryBadge; // Genre badge
        TextView typeBadge; // Content type badge
        TextView episodeBadge; // Episode count badge for series

        public SegmentedMovieViewHolder(@NonNull View itemView) {
            super(itemView);
            ...
            episodeBadge = itemView.findViewById(R.id.episode_badge); // Add this to your layout
        }
    }
