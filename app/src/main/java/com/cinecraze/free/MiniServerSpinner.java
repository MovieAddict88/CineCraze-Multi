...
public class MiniServerSpinner {
    ...
    private void createMiniPopupWindow(Context context) {
        // Create vertical LinearLayout for compact server options
        LinearLayout container = new LinearLayout(context);
        container.setOrientation(LinearLayout.VERTICAL);
        container.setPadding(8, 12, 8, 12); // Increased vertical padding
        ...
        for (int i = 0; i < servers.size(); i++) {
            ...
            serverButton.setTextSize(12); // Slightly larger text for vertical layout
            serverButton.setPadding(16, 12, 16, 12); // Increased padding for better touch targets
            ...
            // Add margin between buttons (vertical spacing)
            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, // Full width for vertical layout
                    ViewGroup.LayoutParams.WRAP_CONTENT
            );
            if (position > 0) {
                params.setMargins(0, 8, 0, 0); // Top margin for vertical spacing
            }
